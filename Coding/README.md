...
判断：有长度为11的哈希表，哈希函数 $H(n) = n \mod 11$，使用链地址法解决冲突，表中已有序列 (9,4,7,13,1,8,23,20,15)，则查找成功的平均次数是 4/3次。

正确。表长为 11，哈希函数为 𝐻(n)= n mod 11。

插入的序列为 (9, 4, 7, 13, 1, 8, 23, 20, 15) 后，每个元素落到对应的槽中。
...
根据各链中元素的分布计算整体平均值。具体计算如下：

- 槽1：长度 2，平均比较次数为 (2+1)/2=1.5 次，贡献 $2 \times 1.5 = 3$次
- 槽2：长度 1，平均比较次数为$(1+1)/2=1$次，贡献$1 \times 1 = 1$ 次
- 槽4：长度 2，平均比较次数为 1.5 次，贡献 3 次
- 槽7：长度 1，贡献 1 次
- 槽8：长度 1，贡献 1 次
- 槽9：长度 2，贡献 3 次
...
判断：红黑树和AVL树搜索时间复杂度都是 $O(\log n)$，且红黑树搜索比AVL树快。

**错误**：红黑树和AVL树的搜索时间复杂度都是 $O(\log n)$，即这两种平衡二叉搜索树都通过保证树的高度不超过 𝑂(log n) 来实现快速查找。但是 AVL 树的平衡性更严格（左右子树高度差最多为1），通常搜索速度会比红黑树快。红黑树采用一种较为宽松的平衡方式，允许子树的高度差异更大，其高度最多为 2×log n。虽然依然保证了 𝑂(log n) 的搜索时间复杂度，但相比AVL树，可能会经过更多节点才能找到目标。红黑树的插入和删除操作更快一些，因为其重平衡操作（旋转和颜色翻转）较少，适合更新频繁的场景。但在纯查找操作上，AVL树由于树更平衡，查找时路径更短，平均情况下搜索速度可能更快。

### 二叉树节点和度

判断：某二叉树只有度为0和度为2的节点，若树高为t，则最少的节点数为2t。

错误。在此题中，“某二叉树只有度为0和度为2的节点”说明该二叉树是**满二叉树**的一种特殊形式（或称“完全二叉树”中的“满二叉树”），即每个非叶子节点都有两个孩子。
 树高$t$常指从根到叶最长路径中节点的个数。最少节点数应为 $1+2\times(t-1) = 2t-1$
...
判断：归并排序、堆排序属于稳定排序，且时间复杂度不会超过 $O(n \log n)$。

错误。

**归并排序（Merge Sort）：**

- **稳定性：** 归并排序是稳定排序算法，因为在合并两个子序列时，当遇到相等元素，通常先将左侧元素复制到辅助数组中，从而保证相等元素的相对顺序不会改变。
- **时间复杂度：** 归并排序的最坏情况、平均情况和最好情况均为 𝑂(n log n)。

**堆排序（Heap Sort）：**

- **稳定性：** 堆排序不是稳定排序算法。由于堆排序在调整堆时可能改变相等元素之间的相对顺序，所以它无法保证相同元素在排序前后的相对位置不变。
- **时间复杂度：** 堆排序的最坏情况时间复杂度为 𝑂(n log n)。

两者都采用分治和堆结构的思想，能在 𝑂(n log n) 的时间内完成排序，但稳定性是评价排序算法的另一个重要指标。

因此，尽管归并排序和堆排序的时间复杂度均不会超过 𝑂(n log n)（实际上是 𝑂(n log n)），但堆排序是不稳定的，所以题目中关于“稳定排序”的表述是错误的。
